...
static int fancy_typecheck(char* tspec, list* arg, char* cmdname,
                           char* cmddesc) {
...
static list* env(list* arg) {
  ...
  if (fancy_typecheck("", arg, "env", "This command prints the environment "
...
static list* list_jobs(list* arg) {
  ...
  for (iter = jobs; iter != NULL; iter = ls_next(iter), i++) {
    ...
    printf("%-3d %-35s %-6d %-6d %-8s\n", i, job->name, job->last_pid,
           job->pgid, (job->status == JOB_STOPPED
                           ? "Stopped"
                           : (job->status == JOB_DEAD ? "Dead" : "Running")));
  }
...
static list* my_if(list* arg) {
  ...
  if (fancy_typecheck("???", arg, "if",
                      "If the \"eval\" of the first argument is an "
                      "\"true\", this command "
...
static list* my_car_l(list* arg) {
  if (fancy_typecheck("*", arg, "car-l", "Simply return the first argument.")) {
    return NULL;
  }
...
static list* my_read(list* arg) {
  ...
  rl_out = readline(ls_data(arg));

  if (!rl_out) return NULL;

  my_out = dynamic_strcpy(rl_out);
  free(rl_out);

  return ls_cons(my_out, NULL);
}
...
static list* my_null(list* arg) { return NULL; }
...
static list* my_true(list* arg) { return ls_copy(ls_true); }

static list* my_false(list* arg) { return ls_copy(ls_false); }
...
static list* my_file_open(list* arg) {
  ...
  if (ret[0] < 0 || ret[1] < 0) {
    error("esh: file-open: couldn't open \"%s\" with mode \"%s\".", name,
          first);
    gc_free(ret);
    return NULL;
  }
...
static list* begin(list* arg) { return ls_copy(arg); }
...
static list* my_while(list* arg) {
  ...
  while (1) {
    ...
    if (exception_flag || (foo && ls_type(foo) == TYPE_BOOL && !ls_data(foo))) {
...
static list* my_void(list* arg) { return ls_copy(ls_void); }
...
hash_entry builtins_array[] = {
  { "cd", cd }, { "help", help }, { "copy", ls_copy }, { "set", set },
  { "get", get }, { "env", env }, { "run", run }, { "run-simple", run_simple },
  { "gobble", gobble }, { "exit", my_exit }, { "alias", alias }, { "+", plus },
  { "*", times }, { "-", minus }, { "/", over }, { "eval", my_eval },
  { "fg", fg }, { "bg", bg }, { "jobs", list_jobs }, { "define", define },
  { "prompt", set_prompt }, { "if", my_if }, { "=", equal_p }, { "pop", pop },
  { "push", push }, { "top", top }, { "list", my_list }, { "print", my_print },
  { "stack", my_stack }, { "hash-make", my_hash_make },
  { "hash-get", my_hash_get }, { "hash-put", my_hash_put },
  { "hash-keys", my_hash_keys }, { "car", my_car }, { "first", my_car },
  { "cdr", cdr }, { "rest", cdr }, { "script", script }, { "read", my_read },
  { "squish", squish }, { "parse", my_parse }, { "newline", newline },
  { "nl", newline }, { "typecheck", my_typecheck }, { "split", split },
  { "unlist", unlist }, { "exec", exec }, { "rot", rot },
  { "l-stack", list_stack }, { "begin", begin }, { "defined?", defined_p },
  { "null", my_null }, { "file-open", my_file_open },
  { "file-read", my_file_read }, { "file-read-block", my_file_read_block },
  { "file-write", my_file_write }, { "file-type", my_file_type },
  { "standard", standard }, { "stderr", my_stderr },
  { "interactive?", my_interactive }, { "and", and }, { "or", or },
  { "not", not }, { "version", version }, { "builtin", builtin },
  { "true", my_true }, { "false", my_false }, { "null?", my_null_p },
  { "not-null?", my_not_null_p }, { "l-cdr", list_cdr }, { "l-rest", list_cdr },
  { "stderr-handler", stderr_handler }, { "wait", my_wait },
  { "alive?", alive_p }, { "while", my_while }, { "alias-hash", alias_hash },
  { "car-l", my_car_l }, { "first-l", my_car_l }, { "chop!", chop },
  { "chop-nl!", chop_nl }, { "match", match }, { "reverse", reverse },
  { "chars", chars }, { "filter", filter }, { "clone", my_clone },
  { "substring?", substring_p }, { "begin-last", begin_last },
  { "<", less_than }, { ">", greater_than }, { "void", my_void },
